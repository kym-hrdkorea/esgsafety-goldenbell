...
expect(countsEqual(exactCounts(items.map((item) => item.item_type)), { OX: 14, MC4: 55, SHORT: 1, ORDER: 2 }), "문항 유형 분포 불일치");
expect(countsEqual(exactCounts(items.map((item) => item.level)), { L1: 26, L2: 35, L3: 11 }), "난이도 분포 불일치");
...
expect(pre.length === 12 && post.length === 12, `사전·사후 측정 수: ${pre.length}+${post.length} (기대 12+12)`);
...
expect(countsEqual(exactCounts(items.filter((item) => item.measure_code).map((item) => item.measure_code)), { M01: 1, M02: 1, M03: 1, M04: 1, M05: 1, M06: 1, M07: 1, M08: 1, M09: 1, M10: 1, M11: 1, M12: 1, M01P: 1, M02P: 1, M03P: 1, M04P: 1, M05P: 1, M06P: 1, M07P: 1, M08P: 1, M09P: 1, M10P: 1, M11P: 1, M12P: 1, T01: 1, T02: 1, T03: 1, T04: 1 }), "측정·전이 코드 분포 불일치");

const sqlCodes = [...itemSql.matchAll(/^ \('([1-6]-[0-9]{2})',/gm)].map((match) => match[1]);
...
for (const [round, season, open, close] of [
  [1, 1, "2026-08-17", "2026-08-21"],
  [2, 1, "2026-08-24", "2026-08-28"],
  [3, 1, "2026-08-31", "2026-09-04"],
  [4, 2, "2026-09-07", "2026-09-11"],
  [5, 2, "2026-09-14", "2026-09-18"],
  [6, 2, "2026-09-21", "2026-09-25"],
]) {
...
console.log("문항은행 검증 통과: 6회차 × 12문항, 72문항, 12쌍 측정, SQL 정합");
