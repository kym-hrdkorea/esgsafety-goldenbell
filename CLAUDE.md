# CLAUDE.md — HRDK 안전·청렴 ON! 골든벨

## 프로젝트 한 줄 정의
한국산업인력공단 전 직원(약 1,800명)을 대상으로 6주간 매주 12문항 안전 퀴즈를 제공하고,
...
9-1. **Supabase MCP가 연결되어 있어도 DDL을 실행하지 않는다.**
   ...
   그러면 `db/05_views.sql`의 뷰와 72문항 시드가 조용히 깨진다.
   스키마 변경이 필요하면 **제안만 하고 멈춘다.**

10. **개인정보를 늘리지 않는다.**
    수집 항목은 사번·닉네임·PIN해시·소속·부서·휴대폰 번호(포상 지급 연락용, 2026-08-20 P항)뿐이다.
    이름·이메일 필드를 추가하지 않는다. 휴대폰 번호는 포상 연락 외 용도로 쓰지 않는다.
    로그에 사번·휴대폰 번호를 남기지 않는다.
