    ...
    <div className="flex flex-col gap-2.5">
      <div className="flex items-center justify-between gap-2.5">
        ...
        <button
          type="button"
          onClick={onReset}
          className="min-h-12 cursor-pointer rounded-[3px] border-2 border-gb-border-card bg-transparent px-4 text-[14px] font-bold text-gb-text-strong-sub active:translate-x-px active:translate-y-px"
        >
          다시 선택
        </button>
      </div>
      {choices.map((label, i) => {
        ...
        return (
          <button
            ...
            type="button"
            onClick={() => onTap(i)}
            className={`flex w-full min-h-[54px] items-center gap-3 rounded border-[3px] px-3.5 py-[13px] text-left text-[17px] leading-[1.5] break-keep active:translate-x-[3px] active:translate-y-[3px] active:shadow-gb-pressed ${
              picked
