...
6회차 전환과 후속 카드 T14~T24의 단일 진입점은 [`continuation-plan.md`](continuation-plan.md)다.
이 문서에는 현재 상태, 72문항 매핑, 성과측정 기준, 사용자 개입 지침, 카드별 완료 조건이 있다.
카드 하나를 완료하고 검증·보고한 뒤 사용자 확인 전에는 다음 카드를 시작하지 않는다.
